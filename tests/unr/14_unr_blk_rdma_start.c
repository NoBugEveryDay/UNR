    ...
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < M; j++) {
            if (dma_type[i][j] == URU_TRANSFER_TYPE_DMA_PUT) {
                unr_blk_rdma_start(send_blk_h[i][j], UNR_NO_SIGNAL, rmt_recv_blk_h[i][j], UNR_NO_SIGNAL, dma_type[i][j]);
            }
            else {
                unr_blk_rdma_start(recv_blk_h[i][j], UNR_NO_SIGNAL, rmt_send_blk_h[i][j], UNR_NO_SIGNAL, dma_type[i][j]);
            }
        }
    }

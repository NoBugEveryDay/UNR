        ...
        for (int j = 0; j < M; j++) {
            ...
            if (dma_type[i][j] == URU_TRANSFER_TYPE_DMA_PUT) {
                unr_blk_part_rdma_start(
                    ...
                    rmt_recv_blk_h[i][j], UNR_NO_SIGNAL, part_offset, dma_type[i][j]);
            }
            else {
                unr_blk_part_rdma_start(
                    recv_blk_h[i][j], UNR_NO_SIGNAL, part_offset, part_size,
                    rmt_send_blk_h[i][j], UNR_NO_SIGNAL, part_offset, dma_type[i][j]);
            }
        }
    }

...
void* recv_buf[N];
void* verf_buf[N];
unr_sig_h send_sig[N], recv_sig[N], rmt_send_sig[N], rmt_recv_sig[N];
unr_sig_h loc_sig_arr[N][M], rmt_sig_arr[N][M];
...
int main(int argc, char** argv) {
    ...
    MPI_Sendrecv(
        send_sig, N, MPI_UNR_SIG_H, 1 - rank, 0,
        rmt_send_sig, N, MPI_UNR_SIG_H, 1 - rank, 0,
        MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    MPI_Sendrecv(
        recv_sig, N, MPI_UNR_SIG_H, 1 - rank, 0,
        rmt_recv_sig, N, MPI_UNR_SIG_H, 1 - rank, 0,
        MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    ...
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < M; j++) {
            if (dma_type[i][j] == URU_TRANSFER_TYPE_DMA_PUT) {
                ...
                loc_sig_arr[i][j] = send_sig[i];
                rmt_blk_h[i][j] = rmt_recv_blk_h[i][j];
                rmt_sig_arr[i][j] = rmt_recv_sig[i];
            }
            else {
                loc_blk_h[i][j] = recv_blk_h[i][j];
                loc_sig_arr[i][j] = recv_sig[i];
                rmt_blk_h[i][j] = rmt_send_blk_h[i][j];
                rmt_sig_arr[i][j] = rmt_send_sig[i];
            }
        }
    }
    for (int i = 0; i < N; i++) {
        unr_blk_part_rdma_batch_plan(
            M,
            &loc_blk_h[i][0], &loc_sig_arr[i][0], &part_offset[i][0], &part_size[i][0],
            &rmt_blk_h[i][0], &rmt_sig_arr[i][0], &part_offset[i][0], &dma_type[i][0], 
            &plan_h[i]);
    }

    {
        int i = rand() % (N-1);
        int j = rand() % M;
        unr_blk_part_rdma_start(
            loc_blk_h[i][j], loc_sig_arr[i][j], part_offset[i][j], part_size[i][j],
            rmt_blk_h[i][j], rmt_sig_arr[i][j], part_offset[i][j], dma_type[i][j] 
        );
    }
    for (int i = 0; i < N; i++) {
